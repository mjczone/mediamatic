...
internal static class SymbolExtensions
{
    ...
    public static IMethodSymbol? FindBy(this ImmutableArray<IMethodSymbol>? methods, MethodBase method)
    {
        ...
        foreach (var methodSymbol in methods)
        {
            ...
            var methodSymbolParams = methodSymbol
                .Parameters.Select(p =>
                    string.IsNullOrEmpty(p.Type.MetadataName)
                        ? p.Type.OriginalDefinition.ToString()?.WithoutNamespaces()
                        : p.Type.MetadataName
                )
                .ToArray();

            if (!methodSymbolParams.SequenceEqual(methodParams.Select(m => m.Name)))
                continue;
    ...
    public static List<Output.ParameterInfo>? GetParameterTypes(this ISymbol symbol)
    {
        ...
        return method
            .Parameters.Select(parameter => new Output.ParameterInfo
            {
                Name = parameter.Name,
    ...
    public static string? GetShortName(this ISymbol symbol)
    {
        switch (symbol.Kind)
        {
            case SymbolKind.Method:
                var method = (IMethodSymbol)symbol;
                return method.MethodKind == MethodKind.Constructor ? method.ContainingType.Name : symbol.Name;
            case SymbolKind.Property:
            case SymbolKind.Event:
                return symbol.Name;
        }
    ...
    public static string? GetDisplayName(this ISymbol? symbol)
    {
        ...
        {
            if (parameterSignature is null)
                return null;
            string[] split = parameterSignature.WithoutNamespaces().Split(' ');
            return split.Length != 2 ? parameterSignature : $"{split[0].GetNullableTypeName()} {split[1]}";
        }
